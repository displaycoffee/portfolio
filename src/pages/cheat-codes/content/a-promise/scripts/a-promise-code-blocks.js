/* Code block snippets */
export const codeBlocks = [
	`<img src="//display.coffee/assets/images/cheat-codes/a-promise-avatar.jpg" alt="A Promise - Avatar" title="A Promise - Avatar" />\n<img src="//display.coffee/assets/images/cheat-codes/a-promise-leaf.png" alt="A Promise - Leaf" title="A Promise - Leaf" />`,
	`import { APromise } from './APromise';`,
	`<APromise />`,
	`<style>\n\u0009/* Copied styles go here */\n</style>`,
	`<!-- Copy styles and paste into stylesheet -->\n<link rel="stylesheet" href="a-promise.css" />`,
	`@import url('a-promise.css');`,
	`import 'a-promise.css';`,
	`.dc-a-promise {\n\u0009max-width: 700px;\n}`,
	`@import 'a-promise.scss';`,
	`import 'a-promise.scss';`,
	`$a-promise-max-width: 700px; // max width`,
];
